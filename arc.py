...
class ARCRecord(object):
    ...
    def from_string(cls, string, version):
		...
		arc_header = ARCHeader(**self.file_headers)
...
class ARCFile(object):
    ...
    def _read_file_header(self):
	    ...
	    try:
	            print "HEADERS SPLIT: %s %s %s %s %s" % (url, ip_address, str(date), content_type, length)
    ...
    def _read_arc_record(self):
		...
		h1 = self.file_headers
		arc_header = ARCHeader(**h1)
	        payload = self.fileobj.read(int(h1['length']))
        #payload = self.fileobj.read(int(headers['length']))
